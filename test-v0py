...
stay_each_stop = st.number_input(label='Período máximo permitido para ficar em cada parada em minutos',
                                 step=1, value=15)
max_time_travel = st.number_input(label='Período máximo de cada viagem em minutos',
                                  step=1, value=150)'''
...
for delivery in deliveries_dict:
    folium.Marker(location=[deliveries_dict[delivery]['lat'], deliveries_dict[delivery]['lon']],
                  ...
                  tooltip=f'Entrega {delivery}').add_to(m)
